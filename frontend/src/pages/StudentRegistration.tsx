...
const StudentRegistration: React.FC<{ isModal?: boolean }> = ({ isModal }) => {
  ...
  const fetchTeams = async (query = '') => {
    ...
    try {
      ...
      const url = query
        ? `${base}/api/teams?query=${encodeURIComponent(query)}&limit=10000`
        : `${base}/api/teams?limit=10000`;
      const res = await fetch(url);
      const ct = res.headers.get('content-type');
      if (!ct?.includes('application/json')) throw new Error(await res.text());
      const result = await res.json();
      setTeams(Array.isArray(result.data) ? result.data : Array.isArray(result) ? result : []);
      setCurrentPage(1);
    } catch (err: any) {
      console.error(err);
  ...
  useEffect(() => {
    ...
    const sub = supabase
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'teams' }, (p) => {
        const u = p.new as Team;
        setTeams((prev) => prev.map((t) => (t.team_id === u.team_id ? { ...t, ...u } : t)));
        setSelectedTeam((prev) => prev?.team_id === u.team_id ? { ...prev, ...u } : prev);
      })
      .subscribe();
    return () => { supabase.removeChannel(sub); };
  }, []);

  const handleSearch = (e: React.FormEvent) => { e.preventDefault(); fetchTeams(searchQuery); };
  const handleCardClick = (team: Team) => { setSelectedTeam(team); setEditMembers(team.team_members || []); };
  ...
  const handleUpdate = async () => {
    ...
    setUpdating(true);
    try {
      const res = await fetch(`${API_BASE_URL}/api/teams/${selectedTeam.team_id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ team_members: editMembers }),
      });
      const ct = res.headers.get('content-type');
      const data = ct?.includes('application/json') ? await res.json() : (() => { throw new Error(res.statusText); })();
      if (res.ok) { setTeams((prev) => prev.map((t) => (t.team_id === data.team_id ? data : t))); setSelectedTeam(null); }
